...
scene = moviemaker.FrameHandler(screen.screen, img_format='.bmp')
mm = moviemaker.MovieMaker(scene.frame_sequence, 'wrapping_wave_function', movie_format='.mkv')
...
while True:
    ...
    first.draw(x, y1, x_scaled=True, y_scaled=True, lw=1)
    first.mention_grid_scale(bg=(252, 225, 225), test_color=(13, 11, 11))
    second.draw(x, y2, x_scaled=True, y_scaled=True, lw=1)
